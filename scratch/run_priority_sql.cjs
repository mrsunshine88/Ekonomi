...
GRANT EXECUTE ON FUNCTION public.toggle_chat_agent(text, boolean, boolean, boolean, integer, integer) TO authenticated;
  ...
  // The safest way to run raw SQL on Supabase using JS client is via a query wrapper if available,
  // or by running a migration via psql. However, Supabase JS client doesn't support raw SQL queries out of the box unless you use an RPC.
  // Wait, I can create an RPC to run SQL, but I need SQL to create the RPC...
  // Let me just write the SQL to a file and instruct the user to run it via Supabase Studio, or since I can't do that,
  // I will write it as an RPC if possible? No.
