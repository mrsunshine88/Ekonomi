...
### Vad
En serie kritiska buggar som gjorde att nya anvÃ¤ndare fastnade i loopar vid godkÃ¤nnande av anvÃ¤ndarvillkor, eller blev inslÃ¤ppta i appen utan konto, har identifierats och Ã¥tgÃ¤rdats. Dessutom har 'DemolÃ¤ge' och 'Live-chatt' lagts till i funktionerna fÃ¶r prenumeranter.

### VarfÃ¶r
NÃ¤r e-postbekrÃ¤ftelse slogs pÃ¥ (och Supabase PKCE-flÃ¶de bÃ¶rjade anvÃ¤ndas) uppstod en kedjereaktion:
1. **Misslyckad Profilskapelse:** Vid registrering skapades ingen session (pga krav pÃ¥ e-postbekrÃ¤ftelse). RLS-regeln fÃ¶r `profiles` krÃ¤vde dock en aktiv session (`auth.uid() = id`) fÃ¶r att fÃ¥ gÃ¶ra en `INSERT`. Detta ledde till att nya konton aldrig fick nÃ¥gon profilrad i databasen.
2. **Loop i Policyn (Terms of Service):** NÃ¤r anvÃ¤ndaren sedan loggade in och godkÃ¤nde policyn, fÃ¶rsÃ¶kte koden uppdatera `tos_accepted = true` pÃ¥ en profil som inte fanns. Felet fÃ¥ngades inte av Supabase utan ignorerades (0 rader uppdaterades). Vid sidladdning trodde appen dÃ¤rfÃ¶r att policyn fortfarande var ogodkÃ¤nd.
3. **Onboarding Bypass:** Koden fÃ¶r att visa 'Skapa HushÃ¥ll'-rutan utvÃ¤rderade `state.accounts.length === 0`. Men vid frÃ¥nkoppling/felaktig profil laddade Zustand `DEFAULT_STATE` (som innehÃ¶ll dummy-konton med `length === 3`). DÃ¤rmed trodde appen felaktigt att anvÃ¤ndaren redan hade konton och slÃ¤ppte in dem i applikationen direkt, utan rÃ¤ttigheter.

### Hur
- **Supabase Auth Trigger:** Skapade SQL-funktionen `handle_new_user` och en databastrigger (`AFTER INSERT ON auth.users`) som automatiskt skapar profilraden pÃ¥ server-sidan med admin-behÃ¶righet (Security Definer), vilket fÃ¶rbigÃ¥r RLS och garanterar att profilen existerar redan innan bekrÃ¤ftelsemailet klickas.
- **RLS fÃ¶r Uppdateringar:** Implementerade `FOR UPDATE` RLS policy pÃ¥ `profiles`-tabellen fÃ¶r att uttryckligen tillÃ¥ta anvÃ¤ndare att uppdatera sina egna rader (t.ex. `tos_accepted` och `role`).
- **FÃ¶rbÃ¤ttrad Gate-logik:** Onboarding-rutan (`needsOnboarding`) i `App.tsx` triggas numera sÃ¤kert av `!householdId` (kontrollerar ifall hushÃ¥ll ID saknas helt) istÃ¤llet fÃ¶r att lita pÃ¥ `accounts.length`. Detta fÃ¶rhindrar helt att dummy-data slÃ¤pper in obehÃ¶riga.
- **FÃ¶rbÃ¤ttrad PKCE URL-detektion:** `AuthContext.tsx` kollar nu efter `code=` och `access_token=` i URL-hashen (samt negerar `type=recovery`) fÃ¶r att sÃ¤kert faststÃ¤lla om det Ã¤r en lyckad e-postbekrÃ¤ftelse (och dÃ¤refter visa 'Grattis din mejl Ã¤r bekrÃ¤ftad'-rutan).
- **Ã…tgÃ¤rdat Databas-schema:** Lade till den saknade `name`-kolumnen (VARCHAR) i `households`-tabellen, vilket eliminerade schema error nÃ¤r anvÃ¤ndare fyllde i namnet pÃ¥ sitt hushÃ¥ll.
- **Ã…tkomst fÃ¶r alla:** Tog bort `role === 'owner'` spÃ¤rren pÃ¥ knapparna till instÃ¤llningsflikarna (`AllmÃ¤nt` och `Konton`) i `ManageBills.tsx`.
- **UI-Uppdatering fÃ¶r SaaS:** Live-chatt och DemolÃ¤ge lades till som punkter i `SubscriptionFeaturesModal.tsx` och `LoginScreen.tsx` fÃ¶r att fÃ¶rtydliga appens vÃ¤rdeerbjudande.
