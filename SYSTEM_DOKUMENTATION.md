...
**Version:** 6.2 (Inbyggd Stripe-motor & Dolt Kassavalv)  
...
| 5.2 | 2026-06-09 | Automatisk Överföring: Ny `isAutoTransfer`-flagga på gemensamma räkningar. Möjliggör val att exkludera specifika personer eller hela räkningen från manuella överföringar. |
...
| **5.9** | **2026-06-10** | **Snygga Modaler & Grundar-UI: Byggde bort alla fula nativa `window.confirm()` på Mina Sidor (radera konto, lämna hushåll, kicka) till förmån för en egen Glassmorphism-modal via React State. Fick lyfta ut modalen från `<div className="card">` via React Fragment `<>` eftersom `backdrop-filter: blur()` i CSS annars bryter fixed positioneringen och fångar upp modalen överst i fönstret istället för skärmen. Lade även till en starkt markerad box över medlemslistan som visar om man är Grundare av hushållet eller bara inbjuden medlem. Om man är grundare (index 0) göms knappen "Lämna hushåll" helt och hållet eftersom man äger sin egen data.** |
| **6.0** | **2026-06-10** | **Premium Excel Export: Bytte ut xlsx (SheetJS CE) mot `exceljs` och `file-saver` för att kunna generera riktigt designade kalkylark. Tabellhuvudet fick en fryst rad, autofilter, och en mörkblå (app-tema) bakgrund med fet, vit text. Data-raderna fick alternerande radfärg (zebra-striping) i ljusgrått och samtliga beloppskolumner formaterades med Excels inbyggda valutaformat (`#,##0 "kr"`). Total-raden är nu avskild med en dubbel överkantlinje och boldad font, vilket ger en betydligt mer polerad och exklusiv företagskänsla på exporten.** |
| **6.1** | **2026-06-10** | **SaaS-Transformation & Betalvägg: Appen är nu en kommersiell produkt. Byggde en interaktiv `Onboarding`-guide (Setup Wizard) för nya kunder att skapa hushåll. Integrerade en Global Master Switch via en ny databastabell (`global_settings`) som styrs från Admin-panelen (hårdkodad säkerhet för grundarens mejl). När betalväggen är aktiv tvingas nya användare möta en `PaywallModal` (simulerad Stripe-kassa 59kr/mån). Lade till en VIP-sökfunktion i Admin-panelen som sätter `stripe_status = 'vip'` i `households`-tabellen via en säker Supabase RPC-funktion, för att ge vänner gratis åtkomst.** |
| **6.2** | **2026-06-10** | **Inbyggd Stripe-motor & Dolt Kassavalv: Uppgraderade betalväggen till skarp drift. Byggde Serverless Functions (`create-checkout`, `stripe-webhook`, `create-portal`) för att kommunicera med Stripes riktiga API. Skapade en högkrypterad tabell `admin_secrets` som endast kan läsas/skrivas via Service Role eller grundarens email, för att undvika miljövariabler i Vercel. Byggde en fristående 👑 `AdminDashboard`-vy i appen där grundaren direkt via UI kan mata in sina `Stripe Secret Key`, `Webhook Secret` och `Price ID`. Betalväggen blockerar nu funktionalitet helt tills Stripe-webkroken asynkront markerar användaren som aktiv.** |
| **6.3** | **2026-06-10** | **PWA Stale Closure Fix & Invite Bypass: Löste en kritisk bugg ("Laddar...") som uppstod när iOS/Android väckte PWA:n ur strömsparläge och Supabase triggade `SIGNED_IN`-event, vilket fångades av en "Stale Closure" i `AuthContext`. Löstes genom att flytta inloggningsspårningen från React State till en fristående `useRef`. Skrev dessutom om `<Onboarding />` (Välkomstguiden) för att erbjuda en Inbjudningskod-ruta direkt på första skärmen. Detta förhindrar nya familjemedlemmar från att tvingas skapa falska hushåll, vilket annars ledde till att de fastnade i betalväggen. Nu kan de ärva grundarens betalda `stripe_status` omedelbart.** |
| **6.4** | **2026-06-10** | **Admin Bypass & Synlig VIP-Lista: Sömlös uppdatering av system-administratörens verktyg. Eftersom aktivering av den Globala Master Switchen ("Betalväggen") omedelbart låste ute även admin (om deras hushåll inte hunnit bli VIP), lades en dold förbikopplingslogik in i `App.tsx` som alltid ger `apersson508@gmail.com` fri lejd. Byggde även in en dynamisk VIP-lista i Admin Dashboard. Admin kan nu se exakt vilka e-postadresser som just nu har VIP-status (tack vare en ny säkerhetsklassad Supabase RPC `get_vip_emails`), och med ett enda knapptryck kan Admin omedelbart dra in VIP-statusen från ett enskilt hushåll via RPC:n `revoke_household_vip_by_email`.** |
| **6.5** | **2026-06-11** | **Användarvillkor, Glömt lösenord & Betalväggsinfo: Implementerade en tvingande ToS & Privacy Policy-modal vid första inloggningen (sparad via `tos_accepted` i tabellen `profiles`). Lade till 'Glömt lösenord'-flöde på inloggningssidan med Supabase `resetPasswordForEmail`. Byggde en 'Vad ingår?'-modal (`SubscriptionFeaturesModal`) på betalväggen som listar alla premiumfunktioner grafiskt och förklarande för nya användare.** |
| **6.6** | **2026-06-11** | **Admin Dashboard Statistik & Top Total Feature: Lade till säker, RLS-bypassad SQL-funktion (`get_admin_stats.sql`) för att live-visa totala medlemmar och betalande hushåll inuti Admin Dashboard. Skapade ny systeminställning under 'Inställningar -> Allmänt' (`showTopTotal`) som valfritt låter användaren slå på en enorm summerad kostnad högst upp i både Månadsvy och Privat Vy. Åtgärdade även en potentiell max-update-depth-loop bugg (React Error #185) i `PrivateView.tsx` via en konvertering från direkt render-set-state till ren `useEffect`.** |
| **6.7** | **2026-06-11** | **Premium Startskärm (Landing Page) & Förbättrad UX: Ersatte den gamla inloggningssidan med en helt nydesignad "Split-screen" landningssida med "Glassmorphism" och dynamiska gradienter för att ge appen ett professionellt SaaS-utseende (14-miljonerskänsla). Responsiv layout säkerställer att inloggningen är i topp-fokus på mobila enheter medan desktop visar säljande informationskort bredvid formuläret. "Logga ut"-knappen flyttades för ökad tillgänglighet till desktop-headern (`App.tsx`) samt hamburgermenyn, och togs bort från Mina Sidor.** |
