...
## 50. Strikt Sp�rning av Demobes�k (Anti-Spam & Datakvalitet)

### Vad
En strikt vitlista, persistent minne och anv�ndarkontroll f�r sp�rning av demobes�k lades till i store.ts och App.tsx.

### Varf�r
Tidigare registrerades demobes�k mer eller mindre s� fort appen sattes i "Demo Mode" och anv�ndaren inte befann sig p� den officiella startsidan. Detta orsakade ett par allvarliga fel i statistiken (Admin Dashboard):
1. Inloggade bes�kare sp�rades: Vid appens uppstart (sidomladdning) uppstod en liten f�rdr�jning medan inloggningssystemet verifierade anv�ndaren (loading = true). Under denna tid v�xlade systemet tillf�lligt till demol�ge i v�ntan p� svar, vilket r�kade trigga ett demo-bes�k f�r inloggade anv�ndare.
2. "Sp�kbes�k" via startsidan: Eftersom logiken litade blint p� att "allt utom startsidan" var en demo-visning, r�ckte det med att bes�karens router-minne i localStorage (t.ex. smartEkonomi_currentView = 'month') lurade appen att bes�karen befann sig p� en demosida - trots att de precis landat p� index-urlen (/).
3. Flera bes�k per session: Om en bes�kare laddade om sidan (F5) i demol�get t�mdes appens lokala React-state (arbetsminnet), vilket nollst�llde variabeln hasLoggedDemoVisit. Samma person registrerades d�rmed upprepade g�nger under samma sittning.

### Hur
- store.ts (S�kerhet & Vitlista): Funktionen logDemoVisit gjordes om till att ta emot den exakta vyn (currentView) som argument. Den v�grar nu att logga ett bes�k om sidan inte finns i vitlistan �ver officiella demosidor (['month', 'stats', 'privat', 'manage', 'mypages']).
- Strikt Anv�ndarsp�rr: En dubbelkontroll (if (get().userId) return;) implementerades f�r att till 100% blockera inloggade fr�n demostatistiken.
- Persistent Sp�rr (sessionStorage): Registreringsstatusen sparas numera i webbl�sarens sessionStorage (has_logged_demo_visit). Oavsett hur mycket bes�karen laddar om eller trycker F5, r�knas de aldrig mer �n en g�ng per session.
- App.tsx (Livscykel): Uppstarten (initCloud) till�ts nu aldrig att k�ra innan AuthContext har returnerat loading = false.
