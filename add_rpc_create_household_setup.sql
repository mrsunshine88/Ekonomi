        ...
        LOOP
            ...
            IF v_bill.normalized_name IS NOT NULL THEN
                INSERT INTO public.global_learning_votes (
                    household_id, normalized_name, transaction_direction, category, source, normalization_version
                )
                VALUES (
                    v_household_id, v_bill.normalized_name, v_bill.transaction_direction::public.transaction_direction_enum, 'BILL'::public.learning_category_enum, COALESCE(v_bill.source, 'ONBOARDING')::public.learning_source_enum, 1
                )
        ...
        LOOP
            ...
            IF v_income.normalized_name IS NOT NULL THEN
                ...
                VALUES (
                    ...
                    COALESCE(v_income.source, 'ONBOARDING')::public.learning_source_enum, 1
                )
                ON CONFLICT (household_id, normalized_name, transaction_direction, category) 
                DO UPDATE SET is_active = true, updated_at = now();
            END IF;
